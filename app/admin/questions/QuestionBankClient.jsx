...
export default function QuestionBankClient({ courses, initialQuestions, initialCourseId }) {
  ...
  return (
    <main className="bank-page tp-fade-in">
      ...
      {draft && (
        <div className="tp-card question-editor tp-fade-in">
          <div className="tp-label" style={{ marginBottom: 14 }}>{editingId ? "Edit question" : "New question"}</div>
            ...
            {draft.options.map((opt, i) => (
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <div
                  ...
                  style={{ width: 18, height: 18, borderRadius: "50%", border: `1.5px solid ${draft.correct_index === i ? "var(--accent)" : "var(--border)"}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}
                >
                  {draft.correct_index === i && <div style={{ width: 8, height: 8, borderRadius: "50%", background: "var(--accent)" }} />}
                </div>
                <input className="tp-input" value={opt} placeholder={`Option ${i + 1}`} onChange={(e) => { const o = [...draft.options]; o[i] = e.target.value; setDraft({ ...draft, options: o }); }} />
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button className="tp-btn tp-btn-primary" disabled={saving} onClick={saveDraft}>{saving ? "Saving…" : "Save question"}</button>
            <button className="tp-btn tp-btn-ghost" onClick={() => { setDraft(null); setEditingId(null); }}>Cancel</button>
          </div>
